...
public class MiniMaxGame {
	// Initialize valus for min and max values
	...
	public static void main(String[] args) {
		...
		// Perform the MiniMax and play the game
		outputPattern o = MiniMaxGamePlay(depth, detailsBoard, 1);
	...
	// Finding optimal move with MiniMax
	public static outputPattern MiniMaxGamePlay(int d, MorrisGameBoardState detailsBoard, int flag) {
		...
		if (d == 0) {
			...
			// Perform static estimation for midgameEndgame phase
			int listsize = l.size();
			if (cntb <= 2) {
				op.data = 10000;
			} else if (cntw <= 2) {
		...
		if (flag == 1) // Flag is true call GenerateMoveMidgameEndgame for White
		{
			...
			op.data = min;
		} else // Flag is false call GenerateMoveMidgameEndgame for Black
		{
